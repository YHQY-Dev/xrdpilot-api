...
type User struct {
	...
	Organization      string                     `json:"organization" gorm:"type:varchar(128)"`
	AcademicIdentity  string                     `json:"academic_identity" gorm:"type:varchar(32)"`
	SupervisorName    string                     `json:"supervisor_name" gorm:"type:varchar(64)"`
	ResearchDirection string                     `json:"research_direction" gorm:"type:varchar(255)"`
	UsagePurpose      string                     `json:"usage_purpose" gorm:"type:text"`
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int, sortOptions ...UserSortOptions) ([]*User, int64, error) {
	...
	// 构建搜索条件
	likeCondition := "username LIKE ? OR email LIKE ? OR display_name LIKE ? OR real_name LIKE ? OR organization LIKE ?"
	likeArgs := []interface{}{
		"%" + keyword + "%",
		"%" + keyword + "%",
		"%" + keyword + "%",
